...
```
this is a lot and I am not 100% sure what each step does
```{r}
eI <- typeI(mu0 = -3, sigma = 2, n = 5000, alternative = "greater", alpha = 0.05)
```
here we get an error of 0.051 which is close to a of 0.05
```{r}
eI <- typeI(mu0 = 5, sigma = 2, n = 1000, alternative = "less", alpha = 0.01)
```
here we get an error of 0.009, which is close to a of 0.01
Typw 1 error is not affected by n or sigma, it is afffected by the change in a because it is defined by a
```{r}
alpha <- 0.05
...
sig  # first 5 values are less than the adjusted alph
```
```{r}
